...
func New(cfg *config.Config) *chi.Mux {
	...
	middleware.InitCORS(r, cfg.FrontendURL)
	...
	orderHandler := handlers.NewOrderHandler(notifHandler)
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Route("/procurement", func(r chi.Router) {
			r.Group(func(r chi.Router) {
				...
				r.Get("/rfqs/market", procurementHandler.ListRFQMarket)
				r.Get("/rfqs/{id}", procurementHandler.GetRFQ)
				r.Post("/rfqs/{id}/images", procurementHandler.UploadRFQImage)
				r.Get("/suppliers", procurementHandler.ListSuppliers)
